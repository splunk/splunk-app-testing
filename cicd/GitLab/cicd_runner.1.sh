...
if [ $# -lt 1 ]; then
      echo "Usage: cicd_runner.sh <registry/image:tag>"
      exit 1
fi
...
echo "Starting splunk image $version..."
docker container create --rm --name splunk --hostname "idx-example.splunkcloud.com" --network testingnet $version
...
#echo "docker cp $CI_PROJECT_DIR/cicd/inputs.conf $version:$APPS_DIR/$APP_ROOT/default/inputs.conf"
#docker cp $CI_PROJECT_DIR/cicd/inputs.conf $version:$APPS_DIR/$APP_ROOT/default/inputs.conf
